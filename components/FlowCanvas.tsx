...
if (node.type === "cropNode") {
  ...
  const res = await fetch("/api/crop", {
    ...
    body: JSON.stringify({
      ...
      cropW: node.data?.cropW ?? "100",
      cropH: node.data?.cropH ?? "100",
    }),
  });
  const data = await res.json();
  const result = { output: data.output };
  ...
  nodeDetails.push({ nodeId: node.id, nodeType: "Crop Node", status: "success", duration: Date.now() - nodeStart });
  completed.set(node.id, result);
  return result;
}

// ================= FRAME =================
// Replace the frameNode section in executeWorkflow with this:
if (node.type === "frameNode") {
  const source = nodeList.find((n) => n.id === incoming[0]?.source);
  if (!source) { setNodeLoading(node.id, false); completed.set(node.id, {}); return {}; }

  // Get video URL from completed map first, then node data
  const video =
    completed.get(source.id)?.output ||
    completed.get(source.id)?.videoUrl ||
    source.data?.output ||
  ...
  console.log("FRAME NODE video:", video?.slice(0, 60), "timestamp:", timestamp);
  ...
  nodeDetails.push({ nodeId: node.id, nodeType: "Frame Node", status: "success", duration: Date.now() - nodeStart });
  completed.set(node.id, result);
  return result;
}
